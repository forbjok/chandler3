...
impl From<ChandlerError> for CliError {
    fn from(error: ChandlerError) -> Self {
        match error {
            ChandlerError::CreateProject(err) => CliError::new(
                CliErrorKind::Config,
                format!("Error creating project: {}", err.to_string()),
            ),
            ChandlerError::LoadProject(err) => CliError::new(
                CliErrorKind::Config,
                format!("Error loading project: {}", err.to_string()),
            ),
            ChandlerError::OpenConfig(err) => CliError::new(
                CliErrorKind::Config,
            ...
            ChandlerError::OpenFile(err) => {
                CliError::new(CliErrorKind::Config, format!("Error opening file: {}", err.to_string()))
            }
            ChandlerError::CreateFile(err) => CliError::new(
                CliErrorKind::Config,
            ...
            ChandlerError::WriteFile(err) => {
                CliError::new(CliErrorKind::Config, format!("Error writing file: {}", err))
            }
