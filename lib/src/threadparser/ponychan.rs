...
impl Iterator for GetReplies {
    type Item = PonychanReply;

    fn next(&mut self) -> Option<PonychanReply> {
        while let Some(node) = self.replies.pop_front() {
            if let Some(reply) = PonychanReply::from_node(node) {
                return Some(reply);
            }
        }
...
impl HtmlDocument for PonychanThread {
...
impl MergeableImageboardThread for PonychanThread {
    type Reply = PonychanReply;

    fn get_all_replies(&self) -> Result<Box<dyn Iterator<Item = Self::Reply>>, ChandlerError> {
        let replies: VecDeque<NodeRef> =
            html::find_elements_with_classes(self.root.clone(), local_name!("div"), &["replyContainer"]).collect();
    ...
    fn merge_replies_from(&mut self, new: Self) -> Result<Vec<Self::Reply>, ChandlerError> {
        ...
        let last_reply_id = if let Some(last_original_reply) = self.get_all_replies()?.last() {
            ...
            let original_op_element =
                html::find_elements_with_classes(self.root.clone(), local_name!("div"), &["opContainer"])
                    .next()
...
mod tests {
    ...
    const THREAD1: &'static str =
        r#"<div class="thread" id="thread_1"><div class="opContainer" id="replyC_1"></div></div>"#;

    // Thread with 2 posts
    const THREAD2: &'static str = r#"<div class="thread" id="thread_1"><div class="opContainer" id="replyC_1"></div><div class="replyContainer" id="replyC_2"></div></div>"#;

    // Thread with post 2 deleted and a new post 3 added
    const THREAD3: &'static str = r#"<div class="thread" id="thread_1"><div class="opContainer" id="replyC_1"></div><div class="replyContainer" id="replyC_3"></div></div>"#;

    // Merged thread with all 3 posts
    const THREAD_MERGED: &'static str = r#"<div class="thread" id="thread_1"><div class="opContainer" id="replyC_1"></div><div class="replyContainer" id="replyC_2"></div><div class="replyContainer" id="replyC_3"></div></div>"#;
    ...
    fn can_merge_threads() {
        ...
        let mut thread1 = PonychanThread::from_document(node1);
        let thread2 = PonychanThread::from_document(node2);
        let thread3 = PonychanThread::from_document(node3);
